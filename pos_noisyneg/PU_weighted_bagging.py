...
def _generate_indices(random_state, bootstrap, weighted_bootstrap, 
                      n_population, n_samples, prob = None):
    """Draw randomly sampled indices."""
    # Draw sample indices
    if bootstrap:
      if weighted_bootstrap and prob is not None:
        indices = random_state.choice(n_population, n_samples,
                                      replace = True, p = prob.reshape(-1,))
      else:
...
def _generate_bagging_indices(random_state, bootstrap_features, 
                              bootstrap_samples, weighted_bootstrap, 
    ...
    feature_indices = _generate_indices(random_state, bootstrap_features, 
                                        weighted_bootstrap, n_features, 
                                        max_features)
    
    sample_indices = _generate_indices(random_state, bootstrap_samples, 
                                       weighted_bootstrap, n_samples, 
                                       max_samples, prob = prob_score)

    return feature_indices, sample_indices


def _parallel_build_estimators(n_estimators, ensemble, X, y, sample_weight,
                               seeds, total_n_estimators, 
                               prob_score, verbose):
    ...
    weighted_bootstrap = ensemble.weighted_bootstrap
...
class BaseBaggingPU(with_metaclass(ABCMeta, BaseEnsemble)):
    ...
    def fit(self, X, y, sample_weight=None, prob_score=None):
    ...
    def _fit(self, X, y, max_samples=None, max_depth=None, sample_weight=None, prob_score=None):
        """Build a Bagging ensemble of estimators from the training
           set (X, y).
        Parameters
        ----------
       ...
       #### WEIGHTED BOOTSTRAP ###############################################
        if prob_score is not None:
            prob_score = check_array(prob_score, ensure_2d=False)
            check_consistent_length(y[y==0], prob_score)
       #### WEIGHTED BOOTSTRAP ###############################################
    ...
    def _get_estimators_indices(self):
        ...
        for seed in self._seeds:
            ...
            feature_indices, sample_indices = _generate_bagging_indices(
                ...
                self.weighted_bootstrap, self.n_features_, len(iU), 
                self._max_features, self._max_samples)
...
class ADBaggingPU(BaseBaggingPU, ClassifierMixin):
    ...
    def __init__(self,
        ...
        super(ADBaggingPU, self).__init__(
            base_estimator,
            n_estimators=n_estimators,
    ...
    def _validate_estimator(self):
        ...
        super(ADBaggingPU, self)._validate_estimator(
